...
from val import test_single_image_BUSI
...
def inference(args, multimask_output, db_config, model, test_save_path=None):
    ...
    with torch.no_grad():
        for case in tqdm(image_list, ncols=70):
            ...
            label = torch.from_numpy(label).unsqueeze(0).float().cuda()
            #print(image.shape)  # 添加批次维度

            # 推理
            metric_i = test_single_image_BUSI(
                image=image,
                label=label,
                net=model,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='BUSI', help='Experiment name')
