...
def patients_to_slices(dataset, patiens_num):
    ...
    if "ACDC" in dataset:
        ...
        ref_dict = {"1": 73, "3": 219, "7": 511}
    elif"BUSI" in dataset:
        ref_dict = {"10": 10, "20": 20, "40": 40,"60": 60}
    else:
...
def trainer_acdc_dualmask_prompt_ssl_fixcoe_random_new_mean_up(args, model, snapshot_path, multimask_output, low_res):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs = model(image_batch, multimask_output, args.img_size, -1)
            ...
            consistency_loss = 0.5 * (ce_loss(outputs1[labeled_bs:], torch.argmax(outputs2[labeled_bs:], dim=1)) + dice_loss(outputs1[labeled_bs:], torch.argmax(outputs2[labeled_bs:], dim=1), softmax=True))

            loss1 = supervised_loss1 + supervised_loss2 + args.coe * consistency_loss
            optimizer.zero_grad()
            loss1.backward()
            optimizer.step()
            ...
            if iter_num < args.warm_iter:
                ...
                outputs_round3 = model(image_batch, multimask_output, args.img_size, 0, args.promptmode_mask)
